...
const createGroup = async (userID, groupName, groupAvatar, members) => {
    try {
        const conversation = await conversationModel.createNewConversation(
            'group',
        );
        if (!conversation) {
            throw new ApiError(
...
const inviteGroup = async (userID, groupID, members) => {
    try {
        ...
        const memberNames = invitedUsers
        ...
        members.forEach((member) => {
            ...
            if (participantSocketAddId) {
                io.to(participantSocketAddId).emit(
                    'newConversation',
                    conversation,
                );
            }
        ...
        const saveMessage = async (uploadResults) => {
            try {
                const messageData = {
                    ...
                    url: JSON.stringify(
                        uploadResults.map((file) => file.Location),
                    ), // Mảng URL
                    type: JSON.stringify(
...
const shareMessage = async (
    ...
    try {
        ...
        if (groupIDs && groupIDs.length > 0) {
            const groupResults = await shareWithGroups(
                userID,
                groupIDs,
                message,
            );
    ...
    for (const receiverId of receiverIds) {
        try {
            ...
            const existingConversation =
                await conversationModel.haveTheyChatted(userID, receiverId);
...
const revokeDeputy = async (userID, participantId, groupID) => {
    try {
        ...
        const isDeputy = groupMembers.some(
            (member) =>
                member.userID === participantId && member.role === 'deputy',
        );
